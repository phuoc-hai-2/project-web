  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>Đơn hàng <strong>#${order._id}</strong> đã được ghi nhận.</p>
        <table border="1" cellpadding="8" style="border-collapse:collapse">
          <thead><tr><th>Sản phẩm</th><th>Số lượng</th><th>Giá</th></tr></thead>
          <tbody>${itemsList}</tbody>
        </table>
        <p><strong>Tổng tiền: ${order.totalPrice.toLocaleString()} VND</strong></p>
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>Dưới đây là mã Key cho đơn hàng <strong>#${order._id}</strong>:</p>
        ${keysList}
        <p>Cảm ơn bạn đã tin tưởng Digital Store!</p>
      `,
    });
  } catch (error) {
    console.error("Email sending error:", error.message);
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>Nhấn vào link bên dưới để đặt lại mật khẩu (có hiệu lực trong 10 phút):</p>
        <a href="${resetUrl}" style="background:#007bff;color:white;padding:10px 20px;text-decoration:none;border-radius:5px">Đặt lại mật khẩu</a>
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>Đơn hàng <strong>#${order._id}</strong> đã được hoàn tiền thành công.</p>
        <p>Số tiền hoàn: <strong>${order.totalPrice.toLocaleString()} VND</strong></p>
        <p>Tiền sẽ được hoàn vào tài khoản của bạn trong 3-5 ngày làm việc.</p>
      `,
    });
  } catch (error) {
    console.error("Email sending error:", error.message);
